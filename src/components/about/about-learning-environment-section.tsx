            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                {GALLERY_IMAGES.map(({ key, src }) => (
                    <div key={key} className="space-y-3">
                        <div className="aspect-video rounded-2xl overflow-hidden shadow-sm">
                            <img src={src} alt={t(`${key}Title`)} className="w-full h-full object-cover" />
                        </div>
                        <div>
                            <h4 className="font-bold text-[var(--color-on-surface)]">{t(`${key}Title`)}</h4>
                            <p className="text-label-sm text-[var(--color-on-surface-variant)]">{t(`${key}Desc`)}</p>
                        </div>
                    </div>
                ))}
                ...
                <div className="space-y-3">
                    <div className="aspect-video rounded-2xl bg-[var(--color-surface-container-low)] border border-[var(--color-surface-variant)] flex items-center justify-center">
                        <Palette className="w-10 h-10 text-[var(--color-primary)]/40" aria-hidden="true" />
                    </div>
                    <div>
                        <h4 className="font-bold text-[var(--color-on-surface)]">{t('env4Title')}</h4>
                        <p className="text-label-sm text-[var(--color-on-surface-variant)]">{t('env4Desc')}</p>
                    </div>
                </div>
            </div>
        </section>
